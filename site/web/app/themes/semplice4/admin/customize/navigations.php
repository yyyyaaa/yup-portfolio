...
if(!class_exists('navigations')) {
	class navigations {
		...
		public function get($mode, $nav_id, $is_editor, $is_crawler) {
			...
			if(!$is_editor) {
				...
				if(!$is_crawler) {
					...
					} else if($post->post_type == 'post' && false !== $blog_navbar && isset($navigations[$blog_navbar])) {
			...
			if(true !== $hide_menu) {
				if(is_array($navigation)) {
					...
					// padding hor
					if(isset($navigation['navbar_type']) && $navigation['navbar_type'] == 'container-fluid' || $navigation['preset'] == 'logo_middle_menu_corners') {
						// padding left
		...
		public function get_header_start($nav_settings, $classes) {
			// vars
			$classes = ' ' . $classes . ' menu-type-' . $nav_settings['menu_type'];
			$mobile_fallback = '';
			// transparent class
		...
		public function get_preset($preset, $nav_settings) {
			...
			switch($preset) {
				...
				case 'preset_five':
					$html .= '
						' . $this->get_header_start($nav_settings, '') . '
							<div class="' . $nav_settings['navbar_type'] . '" data-nav="logo-middle-menu-sides">
								' . $this->get_navbar_inner($nav_settings) . '
									' . $this->get_hamburger(' navbar-right', $nav_settings['menu_type']) . '
