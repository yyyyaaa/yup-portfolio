...
class admin_api {
	...
	public function dashboard($request) {
		...
		if(!is_wp_error($contents) && empty($contents->errors)) {
			...
			// column open
			$output .= '<div class="admin-column dashboard-error" data-xl-width="12"><p>An error occured while trying to fetch content from http://news.semplice.com.</p>';

			if(is_wp_error($contents)) {
				$output .= '<p><span>Error Message: ' . $contents->get_error_message() . '</span></p>';
			} else {
				$output .= '<p><span>Error Message: Unfortunately there were no detailed error message provided.</span></p>';
			}
